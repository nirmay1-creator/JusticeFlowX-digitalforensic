...
async function fetchPackets() {
  try {
    const res = await fetch("https://justiceflowxdigitalforensics-n7tlafs3.b4a.run/api/get_packets");
...
startBtn.addEventListener("click", async () => {
  ...
  try {
    await fetch("https://justiceflowxdigitalforensics-n7tlafs3.b4a.run/api/start_capture", {
      method: "POST",
...
stopBtn.addEventListener("click", async () => {
  try {
    await fetch("https://justiceflowxdigitalforensics-n7tlafs3.b4a.run/api/stop_capture", { method: "POST" });
  } catch(e) {}
...
async function checkStatusOnLoad() {
  try {
    const res = await fetch("https://justiceflowxdigitalforensics-n7tlafs3.b4a.run/api/status");
    const data = await res.json();
    if (data.sniffing) {
